...
.. code-block:: bash
   ...
   npx dts-linter --format --files board.dts --files board_pinctrl.dtsi --outFile diff.patch
   git apply diff.patch

You can omit ``--files`` and this will format all files under the directory where the
command has been called. Alternatively ``--cwd`` can also be passed set the base dir were the tool should look
for files. This path is also used in the to make the path in the patch file relative.
...
`VS Code Marketplace <https://marketplace.visualstudio.com/items?itemName=KyleMicallefBonnici.dts-lsp>`_
`Open VSIX <https://open-vsx.org/extension/KyleMicallefBonnici/dts-lsp>`_
For other editors that support ths LSP Client protocol you can use
