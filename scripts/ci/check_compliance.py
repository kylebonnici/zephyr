    ...
    def run_hello_world_build(self, build_dir: Path):
        ...
        subprocess.run(
            [
                ...
                "-S",
                str(sample_dir),
                "-DMODULES=dts:dts_configuration_files:dts_build_info_output",
                "-P",
...
def _run_tests_sequential(testcases):
    # Runs 'testcases' one after the other. Returns a list of result tuples.
    results = []
...
def _main(args):
    ...
    for testcase, case, fmtd_failures, has_error, has_warning, name in results:
        ...
        # clean: no findings
        if has_error:
            case_levels[name] = "error"
        elif has_warning:
            case_levels[name] = "warning"
        else:
            case_levels[name] = "clean"
